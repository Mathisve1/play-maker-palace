...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 3: Create incident types (idempotent)
    const typesDef = [
    ...
    // Step 4: Create checklist items per zone
    ...
    (async () => {
      ...
      // Phase 1: Complete checklist items over ~2 minutes
      if (totalItems > 0) {
        const delayPerItem = Math.floor(120 / totalItems);
        for (let i = 0; i < totalItems; i++) {
          ...
          } catch (e) { console.error("Checklist progress error:", e); }
        }
      }

      // Phase 2: Wait for GO LIVE (poll every 5s, max 5 min)
      ...
      if (!liveDetected) {
        console.log("GO LIVE not detected within 5 minutes, skipping.");
        return;
      }

      // Phase 3: Simulate 8 incidents over ~3 minutes (every ~22 seconds)
      const incidents = [
        { delay: 8, type: "Medisch", zone: "Hoofdtribune", priority: "medium", desc: "Bezoeker flauwgevallen bij rij 12", lat: 51.0259, lng: 3.7252 },
        { delay: 22, type: "Diefstal", zone: "Parking", priority: "low", desc: "Gestolen rugzak gemeld bij parking P2", lat: 51.0245, lng: 3.7238 },
        { delay: 22, type: "Agressie", zone: "Ingang & Fouillering", priority: "high", desc: "Vechtpartij bij ingang Noord", lat: 51.0263, lng: 3.7261 },
        { delay: 22, type: "Medisch", zone: "Speelveldomgeving", priority: "medium", desc: "Snijwond bij steward tijdens opbouw", lat: 51.0255, lng: 3.7248 },
        { delay: 22, type: "Verdacht pakket", zone: "VIP-lounge", priority: "high", desc: "Onbeheerde tas naast bar VIP", lat: 51.0261, lng: 3.7256 },
        { delay: 22, type: "Brand", zone: "Parking", priority: "high", desc: "Rookontwikkeling bij foodtruck zone C", lat: 51.0242, lng: 3.7234 },
        { delay: 22, type: "Agressie", zone: "Bezoekerstribune", priority: "medium", desc: "Verbale escalatie tussen supporters", lat: 51.0258, lng: 3.7264 },
        { delay: 22, type: "Evacuatie", zone: "Bezoekerstribune", priority: "high", desc: "Gedeeltelijke evacuatie zone B vereist", lat: 51.0257, lng: 3.7266 },
      ];
      ...
      for (const inc of incidents) {
        ...
        } catch (e) { console.error("Incident insert error:", e); }
      }
    })();
