...
const ReportingDashboard = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [tasksRes, eventsRes, signupsRes, paymentsRes, ticketsRes, hourConfsRes, sepaRes,
        ...
        supabase.from('tasks').select('id,title,club_id,event_id,task_date,spots_available,compensation_type,hourly_rate,expense_amount,location,status,partner_only,assigned_partner_id').eq('club_id', clubId).limit(1000),
        ...
        supabase.from('volunteer_payments').select('id,task_id,volunteer_id,amount,status,paid_at,club_id').eq('club_id', clubId).limit(5000),
        supabase.from('volunteer_tickets').select('id,task_id,volunteer_id,status,club_id').eq('club_id', clubId).limit(5000),
        supabase.from('hour_confirmations').select('id,task_id,volunteer_id,status,final_hours,final_amount').limit(5000),
        ...
        supabase.from('compliance_declarations').select('id,volunteer_id,type,status').limit(2000),
        supabase.from('external_partners').select('id,name,category,club_id,is_active').eq('club_id', clubId).limit(200),
        supabase.from('partner_members').select('id,partner_id,user_id,email').limit(2000),
        supabase.from('partner_task_assignments').select('id,task_id,partner_member_id').limit(5000),
        supabase.from('sepa_batches').select('id,reference,status,total_amount,item_count,created_at').eq('club_id', clubId).limit(500),
        supabase.from('loyalty_enrollments').select('id,volunteer_id').limit(2000),
      ]);
  ...
  const handleGenerateSeasonReport = async () => {
    ...
    try {
      const { generateSeasonReport } = await import('@/lib/generateSeasonReport');
