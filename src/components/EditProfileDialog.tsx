        ...
        <div className="space-y-3 mt-6 pt-6 border-t border-border">
          ...
          {stripeAccountId ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between bg-accent/10 border border-accent/20 rounded-xl p-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 px-3 py-2 rounded-xl text-xs font-medium bg-primary/10 text-primary hover:bg-primary/20 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
                >
                  {connectingStripe ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ExternalLink className="w-3.5 h-3.5" />}
                  {language === 'nl' ? 'Wijzigen' : language === 'fr' ? 'Modifier' : 'Manage'}
                </button>
                <button
                  type="button"
                  onClick={async () => {
                    if (!confirm(language === 'nl' ? 'Weet je zeker dat je je Stripe account wilt ontkoppelen?' : language === 'fr' ? 'Êtes-vous sûr de vouloir déconnecter votre compte Stripe ?' : 'Are you sure you want to disconnect your Stripe account?')) return;
                    try {
                      const { error } = await supabase
                        .from('profiles')
                      ...
                      toast.success(language === 'nl' ? 'Stripe account ontkoppeld' : language === 'fr' ? 'Compte Stripe déconnecté' : 'Stripe account disconnected');
                    } catch (err: any) {
                      toast.error(err.message || 'Error');
                    }
                  }}
                  className="px-3 py-2 rounded-xl text-xs font-medium bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors flex items-center justify-center gap-1.5"
                >
                  <AlertTriangle className="w-3.5 h-3.5" />
                  {language === 'nl' ? 'Ontkoppelen' : language === 'fr' ? 'Déconnecter' : 'Disconnect'}
                </button>
              </div>
            </div>
          ) : (
